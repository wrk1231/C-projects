...
int factormodel(double *covariance, double *w, double *averageX, double *eigenvalue, double lambda) {
    ...
    /** read the weight of first year by function 'readweight' **/
    double *previous_weight; //weight of 1st year
    int readcode = 0;
    previous_weight = (double *) calloc(totalassets, sizeof(double));
    readcode = readweight(&previous_weight);


    n = totalassets + lambdanum + totalassets + totalassets + totalassets + totalassets + totalassets; /** 947 'u -> f'& 'v ->g' variables, 10- factor variables,f,g bin **/
    ...
    //set g
    for (j = totalassets*3+lambdanum; j < totalassets*4 + lambdanum; j++) {
        names[j] = (char *) calloc(3, sizeof(char));
        if (names[j] == NULL) {
            retcode = 1;
            goto BACK;
    ...
    //set q
    for (j = totalassets*4+lambdanum; j < totalassets*5+lambdanum; j++) {
        names[j] = (char *) calloc(3, sizeof(char));
        if (names[j] == NULL) {
            retcode = 1;
            goto BACK;
        }
        sprintf(names[j], "q%d", j - totalassets*4-lambdanum);
        obj[j] = 0.0;
        ub[j] = 10.0;
        lb[j] = 0.0;
    }
    ...
    //set xi = ui - vi
    for (j = totalassets*5+lambdanum; j < totalassets*6+lambdanum; j++) {
        names[j] = (char *) calloc(3, sizeof(char));
        if (names[j] == NULL) {
            retcode = 1;
            goto BACK;
        }
        sprintf(names[j], "x%d", j - totalassets*5-lambdanum);
        obj[j] = 0.0;
        ub[j] = 10.0;
        lb[j] = -10.0;
    }
    ...
    for (j = 0; j < n; j++) {
        ...
        else if (j < lambdanum + totalassets*4) vartype[j] = GRB_BINARY;
    ...
    for (int i = 0; i < totalassets; i++) {
        ...
        qrow[i] = i + totalassets*5 + lambdanum;
        qcol[i] = i + totalassets*5 + lambdanum;
    }
...
//        string temp(buffer);
    ...
    /** qi+ui >= previous_weighti **/
    for (j = 0; j < totalassets ; j++) {
        cval[0] = 1.0;
        cind[0] = j;
        cval[1] = 1.0;
        cind[1] = totalassets*4 + j;


        numnonz = 2;

        rhs = previous_weight[j];
    ...
    /** sum of vi <= 0.6 **/
    for (j = totalassets + lambdanum; j < totalassets*2 + lambdanum; j++) {
        cind[j - (totalassets + lambdanum)] = j;
        cval[j - (totalassets + lambdanum)] = 1.0;
    }
    ...
    retcode = GRBaddconstr(model, numnonz, cind, cval, sense, rhs, "Short weight <= 0.6");
    if (retcode) goto BACK;
    ...
    retcode = GRBaddconstr(model, numnonz, cind, cval, sense, rhs, "Short weight >= 0.4");
    if (retcode) goto BACK;
    ...
    for (j = 0; j < totalassets; j++) {
        ...
        cval[2] = -1.0;
        cind[2] = totalassets*5 + lambdanum + j;
    ...
    /** turn over : sum qi <= 0.2**/
    for (j = totalassets*4 + lambdanum; j < totalassets*5 + lambdanum; j++) {
        cind[j-(totalassets*4 + lambdanum)] = j;
        cval[j - (totalassets*4 + lambdanum)] = 1.0;
    }
    ...
    retcode = GRBaddconstr(model, numnonz, cind, cval, sense, rhs, "Turnover <= 0.2");
    if (retcode) goto BACK;
    ...
    for (j = 5*totalassets+lambdanum; j < n; j++) {
        if (x[j] > 0.00001) {
            printf("%s = %g\n", names[j], x[j]);
            numofname += 1;
        }
